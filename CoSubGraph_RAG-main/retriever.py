...
class Retriever(nn.Module):
    def __init__(
        ...
        self.spe = SPE(**SPE_kwargs)

        pred_in_size = 4 * emb_size
        if topic_pe:
            pred_in_size += 2 * 2
        pred_in_size += 2 * 2 * (SPE_kwargs['num_rounds'] + SPE_kwargs['num_reverse_rounds'])
    ...
    def forward(
        ...
        spe_list = self.spe(topic_entity_one_hot, edge_index, reverse_edge_index)
        h_e_list.extend(spe_list)
